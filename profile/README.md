# PowerAI Labs 🚀

## 🌟 Objective

- 🤖 **AI & Machine Learning** – Building intelligent, local-first AI systems.
- ⚡ **Power Platform & Automation** – Enhancing workflows with Power Apps, Dataverse, Power Automate.
- ☁️ **Cloud & On-Prem AI** – Deploying scalable AI solutions locally and in cloud.
- 📚 **RAG & LLMs** – Exploring Retrieval-Augmented Generation and local LLMs.

## 🔥 Featured Work

- **[Local RAG](https://github.com/PowerAI-Labs/Local-RAG)** – A fully local AI-powered document processing and search system.
- **[Learn Fine-Tuning: A Hands-On Course](https://github.com/PowerAI-Labs/Learn-Fine-Tuning-A-Hands-On-Course)** – A comprehensive 19-lesson hands-on course on fine-tuning language models with PyTorch and Hugging Face. Covers foundations, transfer learning, SFT, LoRA/QLoRA, RLHF, DPO, and production deployment.

## 🛠️ Tech Stack

- **AI/ML**: Ollama, FAISS, Local LLMs, PyTorch, Hugging Face Transformers, PEFT, TRL
- **Web & API**: FastAPI, React, Azure
- **Data Processing**: OCR, PDF/DOCX Parsing, Embedding Models

## 📢 Let's Connect!

- **LinkedIn**: [linkedin.com/in/mishraraghav](https://www.linkedin.com/in/mishraraghav)
- **GitHub**: Follow this organization for updates!

💡 *PowerAI Labs is a space for innovation, learning, and building AI-powered solutions together!*
